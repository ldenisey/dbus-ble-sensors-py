...
class BleDevice(object):
    ...
    def handle_mfg(self, manufacturer_data: bytes):
        ...
        if not self.is_enabled():
            logging.info(f"{self._plog} device not enable, skipping")
            return
        ...
        logging.info(f"{self._plog} data parsed = {sensor_data}")
        for dbus_service in self._dbus_services.values():
            # Filtering data
            ...
            # Update sensors data
            logging.info(f"{self._plog} updating data")
            self.update_data(dbus_service, service_data)

            # Update alarm states
            logging.info(f"{self._plog} updating alarms")
            ...
            logging.info(f"{self._plog} connecting service")
...
# /!\/!\/!\/!\/!\/!\  Methods below should not been overrided  /!\/!\/!\/!\/!\/!\
    ...
    def init_settings(self, dbus_service: DbusDeviceService, instance):
        for setting in instance.info['settings']:
            ...
            if (onchange := setting.get('onchange', None)):
                def default(new_value): return logging.error(
                    f"{self._plog} can not find setting {setting['name']} onchange method {onchange}")
    ...
    def load_str(self, reg: dict, manufacturer_data: bytes) -> str:
        if (bits := reg.get('bits', None)) is None:
            logging.warn(f"{self._plog} missing field 'bits' in {reg['name']} reg field. Ignoring value")
            return None
        ...
        if size > len(manufacturer_data) - offset:
            logging.error(f"{self._plog} can not parse {reg['name']}, field is longer than manufacturer data")
            return None
    ...
    def load_int(self, reg: dict, manufacturer_data: bytes) -> any:
        ...
        if (bits := reg.get('bits', None)) is None:
            match _type:
                ...
                case _:
                    return None
        logging.debug(f"{self._plog} reading {bits} bits")
        ...
        logging.debug(f"{self._plog} raw value: {value}")

        # Applying mask, if any
        if reg.get('mask', None) is not None:
            value = value & reg['mask']
            logging.debug(f"{self._plog} value with mask: {value}")

        # Applying shift and triming on bits size
        if shift is not None:
            value = (value >> shift) & ((1 << bits) - 1)
            logging.debug(f"{self._plog} value with shift: {value}")

        # Post actions
        if scale := reg.get('scale', None):
            value = value / scale
            logging.debug(f"{self._plog} value with scale: {value}")
        if bias := reg.get('bias', None):
            value = value + bias
            logging.debug(f"{self._plog} value with bias: {value}")
        if xlate := reg.get('xlate', None):
            value = getattr(self, xlate)(value)
            logging.debug(f"{self._plog} value with xlate execution: {value}")
        if 'REG_FLAG_INVALID' in flags and value == reg.get('inval', None):
            logging.debug(f"{self._plog} value invalidated")
            value = None
        return value
    ...
    def parse_manufacturer_data(self, manufacturer_data: bytes) -> dict:
        ...
        for reg in self.info['regs']:
            logging.debug(f"{self._plog} evaluating reg: {reg}")
            value = None
            match reg['type']:
                ...
                case dbus.types.ObjectPath | dbus.types.Signature | dbus.types.Array | dbus.types.Dictionary | dbus.types.ByteArray | dbus.types.Variant:
                    logging.warn(f"{self._plog} can not parse data type {reg['type']}, ignoring value.")
                case _:
                    pass

            logging.debug(f"{self._plog} parsed: {reg['name']}={value}")
            if value is None:
                continue
            ...
            for role in roles:
                if values.get(role, None) is None:
                    logging.error(f"{self._plog} role '{role}' from reg '{reg['name']}' is unknown, ignoring it")
                else:
    ...
    def update_alarm(self, dbus_service: DbusDeviceService, alarm: dict, instance):
        ...
        if alarm.get('flags', None) and 'ALARM_FLAG_HIGH' in alarm['flags']:
            ...
            sensor_value < level
        logging.debug(f"{self._plog} updating alarm, sensor_value={sensor_value} level={level} active={active}")
