...
class BleRoleMovement(BleRole):
    ...
    Defaut alarm is 'Movement', which indicates if movement has been detected. It requires either a 
    'MovementState' or a 'MovementCount' item from the device.

    This role is not officialy supported by Venus OS, hence there is no UI not alarm support for it.
    ...
    def get_alarm_movement(self, role_service) -> int:
        if role_service['Alarms/Movement/Enable'] is False:
            return 0
        if (movement_state := role_service['MovementState']):
