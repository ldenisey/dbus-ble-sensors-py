...
from conf import PROCESS_NAME, PROCESS_VERSION, DBUS_DEVICE_SERVICE_TIMEOUT
...
class DbusDeviceService(object):
    ...
    async def _connection_timeout(self):
        await asyncio.sleep(DBUS_DEVICE_SERVICE_TIMEOUT)
        logging.warning(
            f"{self._ble_device._plog} no data received since {DBUS_DEVICE_SERVICE_TIMEOUT} seconds, disconnecting dbus service")
        self.disconnect()
    ...
    @staticmethod
    def _clear_setting_path(clear_path: str) -> str:
        if clear_path.startswith('/Settings'):
            return clear_path[9:]
        return clear_path

    def get_value(self, path: str) -> any:
        return self._dbus_service._dbusobjects.get(self._clear_path(path), None).local_get_value()
    ...
    def init_custom_name(self):
        self._init_proxy_setting(
            f"/Settings/Devices/{self._dev_id}/{self.ble_role.get_name()}/CustomName",
            '/CustomName',
            '',
        )

    def set_device_name(self):
        self.set_value('/DeviceName', self._ble_device.info['DeviceName'])
    ...
    def set_alarm_active_status(self, alarm: dict, active: bool):
        state_path = self._get_alarm_active_status_path(alarm)
        self.set_value(state_path, active)
